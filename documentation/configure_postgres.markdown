...
that users installing OpenVoxDB via [the module][module] will already have
...
PostgreSQL on a different server from your OpenVoxDB node, you must [configure
an SSL connection between OpenVoxDB and PostgreSQL](#using-ssl-with-postgresql)
to secure your database connections. Otherwise your OpenVoxDB communication with
...
permissions of OpenVoxDB queries and prevents them from writing, updating,
...
Particularly if you plan to run more than one OpenVoxDB instance
connecting to the same database, we recommend you also
[add and use a "migrator" user](#coordinating-database-migrations)

Install the [`pg_trgm`][pg_trgm] extension. OpenVoxDB makes use of this
...
Postgres and OpenVoxDB. It is also possible to use password authentication, but
this has many limitations. Firstly, your PostgreSQL database username and
password will be stored in plaintext in your OpenVoxDB configuration files.
Additionally, neither Postgres nor OpenVoxDB will be verifying the identity of
...
If you plan to run more than one OpenVoxDB instance connected to the
...
that an OpenVoxDB server never tries to use a database whose migration
level (data format version) differs from the one it expects.

OpenVoxDB will refuse to start if it detects an unexpected migration
level, which covers many cases, but won't help, for example, if a new
version of OpenVoxDB is started while older versions are still running.
...
invalid/out-of-date) OpenVoxDB instances continue to access the
database.  That's true even though newer OpenVoxDB versions have a
check to prevent them from creating new connections to an unrecognized
database version because OpenVoxDB can continue to use any connections
...
provide OpenVoxDB with a separate, suitably configured PostgreSQL user
(role), for migrations.  That role must have the ability to grant and
revoke connection privileges to/from the normal OpenVoxDB database
