...
* You're trying to talk over plain text (8080) and OpenVoxDB's not listening.

By default, OpenVoxDB only listens for plain text connections on localhost, for
security reasons. In order to talk to it this way, you'll need to either
forward the plain text port or change the interface OpenVoxDB binds on to one
that is accessible from the outside world. In the latter case, you'll want to
use some other means to secure OpenVoxDB (for instance, by restricting which
hosts are allowed to talk to OpenVoxDB through a firewall).

* You're trying to talk over SSL and nobody trusts anybody else.

Because OpenVoxDB uses the certificate authority (CA) of your OpenVox
infrastructure, and a certificate signed by it, OpenVoxDB doesn't trust your
browser, and your browser doesn't trust OpenVoxDB. In this case, you'll need to
give your browser a certificate signed by your OpenVox CA. Support for client
certificates is varied between browsers, so it's preferred to connect over
plain text, as outlined above.

## Does OpenVoxDB support `puppet apply`?

Yes. However, the setup is quite different from the normal OpenVox Server based setup, so
[consult the documentation][connect_puppet_apply] for more details.

## Why is OpenVoxDB written in Java?

Actually, OpenVoxDB isn't written in Java at all! It's written in a language
...
The error can be seen in your PostgreSQL log and the OpenVoxDB log and looks like the output below.
...
## Why is OpenVoxDB using so much CPU?

There are numerous possible contributing factors to high CPU usage by OpenVoxDB,
both on the application server and (if different) the database. Examples
include the total number of nodes managed by OpenVox, the frequency of the agent
...
## My OpenVox Server is running slower since I enabled OpenVoxDB. How can I profile it?

The openvoxdb-termini client code includes profiling capability. By simply
adding `profile=true` to your `puppet.conf`, you can enable detailed profiling
of all aspects of OpenVox, including openvoxdb-termini. For this to work, you
must enable debugging on your OpenVox Server instance as well.
...
When OpenVoxDB is running in a "steady state", it should have a very
...
OpenVoxDB can't keep up with the work that is being enqueued. This is a
good indication that some performance tuning needs to take
place. There are several areas to consider when performance
tuning. OpenVoxDB is sensitive to PostgreSQL performance issues, so
...
processed concurrently. If OpenVoxDB is consuming too many resources on
a shared system, this number can be reduced. For servers that are
dedicated OpenVoxDB instances, setting this value to the number of
logical cores could significantly improve command
throughput. Increasing the number of threads should also be paired
with increasing the [amount of memory allocated to OpenVoxDB][java heap].
...
faster puppet runs as the OpenVoxDB terminus enqueues the message as
...
reduce the throughput on the OpenVoxDB instance. That could potentially
