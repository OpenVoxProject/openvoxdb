...
[resources_type]: https://docs.openvoxproject.org/openvox/latest/type.html#resources
[logback]: ./configure.markdown#the-logback-logging-config-file
[dashboard]: #monitor-the-performance-dashboard

OpenVoxDB requires a relatively small amount of maintenance and tuning. However, you should become familiar with the occasional tasks outlined in this guide.

## Monitor the performance dashboard

OpenVoxDB hosts a performance dashboard on port 8080, which listens only on
...
* `pollingInterval`: how often to poll OpenVoxDB for updates, in milliseconds
...
When you remove a node from your OpenVox deployment, it should be marked as **deactivated** in OpenVoxDB. This will ensure that any resources exported by that node will stop appearing in the catalogs served to the remaining agent nodes.

* OpenVoxDB can automatically mark nodes that haven't checked in recently as **expired**. Expiration is simply the automatic version of deactivation; the distinction is important only for record keeping. Expired nodes behave the same as deactivated nodes. By default, nodes are expired after 7 days of inactivity; to configure this, use the [`node-ttl` setting][node_ttl].
...
* Any deactivated or expired node will be reactivated if OpenVoxDB receives new catalogs or facts for it.
...
If you are saturating your CPU, we recommend [lowering the number of threads][threads].  This prevents other OpenVoxDB subsystems (such as the web server, or the MQ itself) from being starved of resources and can actually _increase_ throughput.

## Redo SSL setup after changing certificates

If you've recently changed the certificates in use by the OpenVoxDB server, you'll also need to update the SSL configuration for OpenVoxDB itself.

If you've installed OpenVoxDB from OpenVox packages, you can simply re-run the
