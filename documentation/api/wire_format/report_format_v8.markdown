...
String. The version of OpenVox that was run to generate this report.

### `report_format`

Integer. The version number of the report format that OpenVox used to generate
the original report data. This is a constant defined by OpenVox.

### `configuration_version`

String. An identifier that OpenVox uses to match a specific catalog for a node
to a specific OpenVox run. This value is generated by OpenVox.

### `start_time`

DateTime. The time on the agent at which the OpenVox run began; see more details
about the `datetime` format below.

### `end_time`

DateTime. The time on the agent at which the OpenVox run completed; see
more details about the `datetime` format below.

### `producer_timestamp`

DateTime. The time of catalog submission from the OpenVox Server to
OpenVoxDB. This field is currently populated by the OpenVox Server. See more details
about the `datetime` format below.

### `producer`

String. The certname of the OpenVox Server that sent the report to
OpenVoxDB. This field may be `null`.

### `transaction_uuid`

String. A string used to match the report to an OpenVox run, and when not using
...
String. A string used to match a report to an OpenVox run initiated by an
orchestration service. This field may be `null`.

### `cached_catalog_status`

String. Used to represent whether the OpenVox run used a cached catalogs and
whether or not the cached catalog was used due to an error or not. This field
may be `null`.

### `status`

String. The status of the OpenVox run.

### `noop`

Boolean. Indicates whether the report was resulted from a `noop` OpenVox run.
...
`"resource_title"` is the title of the OpenVox resource that the event occurred on.
...
OpenVox run; a skipped resource should have an empty list of `"events"`.

`"containment_path"` is a collection of strings where each string is an OpenVox type or class that represents the containment hierarchy of the resource within the catalog. This field may be `null`.

`"events"` is a collection of objects where each is an OpenVox event corresponding
...
`"old_value"` is the value that OpenVox determined the property to have held prior
to the event.

`"new_value"` is the value that OpenVox was instructed to set the property to.
