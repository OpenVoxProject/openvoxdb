...
`"puppet_version"` is the version of OpenVox that was run to generate this report.

`"report_format"` is the version number of the report format that OpenVox used
to generate the original report data. This is a constant defined by OpenVox.

`"configuration_version"` is an identifier string that OpenVox uses to match a
specific catalog for a node to a specific OpenVox run. This value is
generated by OpenVox.

`"start_time"` is the time on the agent at which the OpenVox run began; see more
details about the `datetime` format below.

`"end_time"` is the time on the agent at which the OpenVox run completed; see
more details about the `datetime` format below.

`"producer_timestamp"` is the time of catalog submission from the OpenVox Server to
OpenVoxDB. This field is currently populated by the OpenVox Server. See more details
about the `datetime` format below.

`"transaction_uuid"` is a string used to identify an OpenVox run. It can be used to
...
report during an OpenVox run. It can be used to match a report with the catalog
...
`"cached_catalog_status"` is a string used to identify whether the OpenVox run
used a cached catalogs and whether or not the cached catalog was used due to an
error or not. This field may be `null`.

`"status"` is a string used to identify the status of the OpenVox run.
